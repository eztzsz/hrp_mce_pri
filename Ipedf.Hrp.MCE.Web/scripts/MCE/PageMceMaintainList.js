...
app.controller('index', ['$scope', '$http', '$rootScope', 'ParamService', 'ParamS', 'InputschangeState',
 function ($scope, $http, $rootScope, ParamService, ParamS, InputschangeState) {
     ...
     $scope.getModelList = function (curIndex) {
         ...
         $http({
             method: "GET",
             url: $rootScope.baseUrl + "service/mce/mce_mt/Srv_MCE_GetMCEMaintainList.ashx" + param
         }).then(function (data) {
             var data = data.data;
             if (data.Succeed == 1) {
     ...
     $scope.goDetail = function (id) {
         window.location.replace("PageMceMaintainNew.html?id=" + id)
     };
     ...
     $scope.deleteModel = function () {
         ...
         if (list == null || list.length == 0) {
             ...
             $http.post($rootScope.baseUrl + "service/mce/mce_mt/Srv_MCE_MCEMaintainOperate.ashx?oType=delete", list).success(function (data) {
                 if (data.Succeed == 1) {
                     alert("删除成功！");
                     $scope.getModelList();
                 } else {
