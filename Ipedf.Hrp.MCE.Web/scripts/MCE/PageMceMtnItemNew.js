...
app.controller('index', ['$scope', '$http', '$rootScope', 'ParamService', 'ParamS', 'InputschangeState',
    function ($scope, $http, $rootScope, ParamService, ParamS, InputschangeState) {
        ...
        $scope.getDetailInfo = function () {
            $http({
                method: "GET",
                url: $rootScope.baseUrl + "service/mce/mce_mt/Srv_MCE_GetMCEMTNItemDetail.ashx?id=" + $scope.paramObj.id
            }).then(function (data) {
                var data = data.data;
                if (data.Succeed == 1) {
                    ...
                    $scope.headInfo.ATTENTION = data.Message.head.ATTENTION;
                    $scope.headInfo.MTNMETHOD = data.Message.head.MTNMETHOD;
        ...
        $scope.saveModel=function (vType) {
            ...
            $http({
                method:"POST",
                url: $rootScope.baseUrl + "service/mce/mce_mt/Srv_MCE_MCEMTNItemSave.ashx",
                data: $scope.headInfo
            }).then(function (data) {
                var data = data.data;
                if(data.Succeed == 1){
        ...
        $scope.goBack = function () {
            window.location.replace("PageMceMTNItemList.html")
        };
