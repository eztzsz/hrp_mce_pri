...
app.controller('index', ['$scope', '$http', '$rootScope', 'ParamService', 'ParamS', 'InputschangeState',
 function ($scope, $http, $rootScope, ParamService, ParamS, InputschangeState) {
     ...
     $scope.goDetail = function (id) {
         window.location.replace("PageMceOutstockNew.html?id=" + id)
     };
     ...
     $scope.deleteModel = function () {
         ...
         if (list == null || list.length == 0) {
             ...
             $http.post($rootScope.baseUrl + "service/mce/mce_outstock/Srv_MCE_MCEOutstockOperate.ashx?oType=delete", list).success(function (data) {
                 if (data.Succeed == 1) {
                     alert("删除成功！");
                     $scope.getModelList();
                 } else {
